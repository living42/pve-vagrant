...
VERSION=${VERSION:-10.12.2-20220419}
...
wget https://cdimage.debian.org/cdimage/openstack/$VERSION/SHA256SUMS
wget --progress dot:giga -N https://cdimage.debian.org/cdimage/openstack/$VERSION/debian-$VERSION-openstack-amd64.qcow2

sha256sum -c SHA256SUMS --ignore-missing

qm create $VMID --name $TEMPLATE_NAME -ostype l26 -cpu cputype=kvm64 --memory 2048 --net0 virtio,bridge=vmbr0 --ipconfig0 ip=dhcp

qm set $VMID -ostype l26

qm importdisk $VMID debian-$VERSION-openstack-amd64.qcow2 local-lvm -format qcow2
